...
const int NumAndGates = 1;
const int NumOrGates = 1;
const int NumXorGates = 1;
const int NumWires = 4;                                                     
const int MaxBinaryInput = 3;
...
//const eLogicLevel LogicList[] = {LOGIC_LOW, LOGIC_HIGH};                    // Enum list of logic levels to simply testing
...
class CHalfAdder
{
    ...
    public:
        CHalfAdder();                                                               // Constructor initalising the connections for a half adder
...
class CParallelAdder: public CHalfAdder
{
    private:
        ...
        // List of Logic Levels that have been converted from a string
        eLogicLevel FirstNumber[MaxBinaryInput];                                    
        eLogicLevel SecondNumber[MaxBinaryInput];

    public:
        // Get user input
        void ObtainInput(eLogicLevel FirstNumber[MaxBinaryInput],eLogicLevel SecondNumber[MaxBinaryInput] );                 

        // Convert user inputs
        void ConvertInputs(std::string FirstInputNumber, std::string SecondInputNumber);
...
{      
    ...
    // Get user inputs to be used
    ParallelAdder.ObtainInput(FirstNumber, SecondNumber);
...
void CParallelAdder::ObtainInput(eLogicLevel FirstNumber[MaxBinaryInput], eLogicLevel SecondNumber[MaxBinaryInput])
{
...
// Taking both converted inputs from the user and computing the output
